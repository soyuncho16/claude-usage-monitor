...
| Windows 트레이 | C#/.NET 8 WinForms NotifyIcon | 동작(코어 로직 CI, 트레이 실기) | ClaudeUsage.Core dotnet test(Linux/CI) + Tray windows 빌드 + 작성자 실기 검증 |
...
4. Windows 트레이 앱 — 완료 (.NET 8 WinForms; Core dotnet test + Tray CI 빌드, 작성자 실기 검증)
